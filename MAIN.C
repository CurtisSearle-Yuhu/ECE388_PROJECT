...
int main() // main function
{
	...
	Init_ADC(); // initialize the Analog to Digital convector (Potentiometer)tg
	int Humiture; // define Heat Index
...
/*---------- START GO BACK TO HELLO ----------*/						
		...
		{	
			...
			_delay_ms(3000);
/*---------- START DISPLAY EEPROM ----------*/	
			if (PINE != (1<<3) && X0 < 10)
			{
				lcd_WriteCommand(0x01); // CLEAR LCD for new message
				lcd_GoToXY(0,2); // position on LCD first column and first row (settings is backwards)
				lcd_PrintString("Loading Previous"); // instruct the user with a displayed message
				lcd_GoToXY(0,1); // position on LCD first column and first row (settings is backwards)
				lcd_PrintString("Saved Data:"); // instruct the user with a displayed message
				_delay_ms(2500);
			...
			{		
				...
				eeprom_busy_wait();
					...
					{
						eeprom_read_block(string,1,strlen(string));
						lcd_GoToXY(0,2);
					...
					{
						eeprom_read_block(string,11,strlen(string));
						lcd_GoToXY(0,2);
					...
					{
						eeprom_read_block(string,21,strlen(string));
						lcd_GoToXY(0,2);
					...
					{
						eeprom_read_block(string,31,strlen(string));
						lcd_GoToXY(0,2);
					...
					{
						eeprom_read_block(string,41,strlen(string));
						lcd_GoToXY(0,2);
						lcd_PrintString("Saved Data5:          ");
						lcd_GoToXY(0,1);
						lcd_PrintString(string);
					}
					if (TempSetPref >=40 && TempSetPref<50)
					{
						eeprom_read_block(string,51,strlen(string));
						lcd_GoToXY(0,2);
						lcd_PrintString("Saved Data6:          ");
						lcd_GoToXY(0,1);
						lcd_PrintString(string);
					}
					if (TempSetPref >=30 && TempSetPref<40)
					{
						eeprom_read_block(string,61,strlen(string));
						lcd_GoToXY(0,2);
						lcd_PrintString("Saved Data7:               ");
						lcd_GoToXY(0,1);
						lcd_PrintString(string);
					}
					if (TempSetPref >=20 && TempSetPref<30)
					{
						eeprom_read_block(string,71,strlen(string));
						lcd_GoToXY(0,2);
						lcd_PrintString("Saved Data8:             ");
						lcd_GoToXY(0,1);
						lcd_PrintString(string);
					}
					if (TempSetPref >=10 && TempSetPref<20)
					{
						eeprom_read_block(string,81,strlen(string));
						lcd_GoToXY(0,2);
						lcd_PrintString("Saved Data9:           ");
						lcd_GoToXY(0,1);
						lcd_PrintString(string);
					}
					if (TempSetPref >=0 && TempSetPref<10)
					{
						eeprom_read_block(string,91,strlen(string));
						lcd_GoToXY(0,2);
						lcd_PrintString("Saved Data10:          ");
						lcd_GoToXY(0,1);
						lcd_PrintString(string);
					}
					if (PINE != (1<<3))
					{
						goto HIGH_T; // go back to high temp select
					}
/*---------- END DISPLAY EEPROM ----------*/
				}
...
/*---------- END LOW TEMPERATURE ----------*/
		...
		{
			_delay_ms(1000); // wait a set time 1 second
/*---------- START GO BACK TO HIGH TEMPERATURE ----------*/
			if (PINE != (1<<3)) // if push button is held down
			{
				lcd_WriteCommand(0x01); // clear display (LCD)
				lcd_GoToXY(2,2); // position on LCD first column and first row (settings is backwards)
...
/*---------- END GO BACK TO HIGH TEMPERATURE ----------*/
/*---------- START RESET PROGRAM ----------*/
				if (PINE != (1<<3)) // inactive state the button is reading high
				{
					lcd_WriteCommand(0x01); // CLEAR LCD for new message
				...
				goto HIGH_T; // return to the welcome message
			}
/*---------- END RESET PROGRAM ----------*/			
/*---------- START TEMPETURE PREFERENCE CHECK ----------*/
if (X0 < X1 || X1 > X0 )
{
	while(X0 < X1)
	{
...
/*---------- END TEMPETURE PREFERENCE CHECK ----------*/
/*---------- START HIGH HUMIDITY ----------*/
...
/*---------- END HIGH HUMIDITY ----------*/
		...
		{
			_delay_ms(1000); // wait a set time 1 second
/*---------- START GO BACK TO LOW TEMPERATURE ----------*/
			if (PINE != (1<<3)) // if push button is held down
			{
				lcd_WriteCommand(0x01); // clear display (LCD)
				lcd_GoToXY(2,2); // position on LCD first column and first row (settings is backwards)
...
/*---------- END GO BACK TO LOW TEMPERATURE ----------*/
/*---------- START RESET PROGRAM ----------*/
				if (PINE != (1<<3)) // inactive state the button is reading high
				{
					lcd_WriteCommand(0x01); // CLEAR LCD for new message
				...
				goto LOW_T; // return to the welcome message
			}
/*---------- END RESET PROGRAM ----------*/
/*---------- START LOW HUMIDITY ----------*/
...
/*---------- END LOW HUMIDITY ----------*/
		...
		{
			_delay_ms(1000); // wait a set time 1 second
/*---------- START GO BACK TO HIGH HUMIDITY ----------*/
			if (PINE != (1<<3)) // if push button is held down
			{
				lcd_WriteCommand(0x01); // clear display (LCD)
				lcd_GoToXY(2,2); // position on LCD first column and first row (settings is backwards)
...
/*---------- START RESET PROGRAM ----------*/
				...
				goto HIGH_H; // return to the welcome message
			}
/*---------- END RESET PROGRAM ----------*/
/*---------- START HUMIDITY PREFERENCE CHECK ----------*/
if (Y0 < Y1 || Y1 > Y0 )
{
	while(Y0 < Y1)
	{
...
/*---------- END HUMIDITY PREFERENCE CHECK ----------*/
/*---------- START DISPLAY PREFERENCES ----------*/	
if(X0 == 100)
{
	pref: 
	eeprom_pref_saved(&X0,&X1,&Y0,&Y1);
}
	eeprom_save_pref(X0,X1,Y0,Y1);
...
/*---------- END DISPLAY PREFERENCES ----------*/
/*---------- START DHT11 MONITORING ----------*/	
	while(1) // open 9th while loop
	{
		while(1) // open 10th while loop
		{
			...
			else
			{
				...
				Humiture = (-42.379 + 2.04901523*I_Temp + 10.14333127*I_RH- 0.22475541*I_Temp*I_RH - 0.00683783*I_Temp*I_Temp - .05481717*I_RH*I_RH + .00122874*I_Temp*I_Temp*I_RH + 0.00085282*I_Temp*I_RH*I_RH - 0.00000199*I_Temp*I_Temp*I_RH*I_RH)+1.3;
